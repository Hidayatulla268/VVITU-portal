...
### CC. Real-Time Class Transfer, Proxy Conduction & Period Attendance Synchronization Engine
*   **What it is:** A high-precision period transfer and substitute attendance authorization mechanism. Supports both **Faculty Peer Substitutions** (arranged between colleagues) and **Official Proxies** (designated by HOD or College Administration).
*   **How it Works:** When a class transfer or proxy assignment is created, the substitute faculty member is authorized to take the class and mark attendance for that exact slot. When attendance is saved by the substitute teacher, the system automatically:
    1. Records `marked_by = substitute_faculty` on all individual student `Attendance` rows.
    2. Updates the `ClassTransfer.status` from `accepted` to `completed`.
    3. Seamlessly binds any logged `ClassDiary` topics to the substitute faculty while updating the subject's curriculum progress.
    4. Automatically flags direct proxy conduction in history matrices even if a pre-arranged transfer record was not created beforehand (`marked_by != slot.faculty`).

### DD. Department & Institutional Class Attendance Audit Console
*   **What it is:** Period-by-period class conduction and student attendance monitoring consoles for HODs (`/hod/faculty-class-audit/`) and University Administrators (`/admin-portal/faculty-class-audit/`).
*   **Architecture & Aggregation:**
    *   Pre-combines regular scheduled day slots with extra transferred/proxy slots **before** querying attendance, ensuring transferred classes immediately show attendance submitted with accurate present/absent student counts.
    *   Computes real-time compliance metrics: Total Scheduled Classes, Marked Sessions, Pending Unmarked Classes, Official Proxies, Faculty Peer Substitutions, and Overall Institutional Compliance Percentage.
    *   Provides deep-dive session audit cards (`class_attendance_detail`) with period attendance rosters, biometric presence status of assigned teachers, and one-click printable class registers.

### EE. Timetable Modifications, Free Faculty Allocation & Proxy Reversal Control
*   **What it is:** An interactive timetable and substitute assignment console featuring automated conflict resolution and instantaneous proxy cancellation.
*   **Key Capabilities:**
    *   `get_free_faculty_for_period(date, period, department, exclude_faculty)` checks the weekly timetable, active proxy assignments, and approved faculty leaves to return ONLY conflict-free teachers.
    *   `cancel_proxy` action in both HOD (`/hod/class-transfers/`) and Admin (`/admin-portal/faculty-class-history/`) history tables allows instantaneous cancellation of proxy transfers with automatic reversion to the regular timetable instructor.

### FF. Student Detention & Junior Readmission Resolution Architecture
*   **What it is:** An institutional workflow managing students detained due to attendance shortage (<65%) or credit shortage.
*   **Workflow:**
    1. Students apply for junior batch readmission (`StudentReadmissionRequest`) specifying their previous year/section, target junior year/section, and academic justification.
    2. The Department HOD reviews and recommends approval (`hod_status = 'approved'`).
    3. College Administration performs final verification and executes `execute_readmission()`, which:
       - Transitions the student's status to `READMITTED` and clears `is_detained = False`.
       - Reassigns the student to the junior Year and Section.
       - Dispatches SMS notifications to the student and parents.

### GG. Student On-Duty (OD) & Medical Leave Exemption Engine
*   **What it is:** A formal student leave and on-duty attendance exemption mechanism (`StudentLeaveRequest`).
*   **Automatic Historical Attendance Sync:** When an OD or Medical leave is approved by HOD or Admin, the engine automatically retroactively updates all student attendance records across the leave date range from `Absent ('A')` to `Leave / On-Duty ('L')`, safeguarding the student's eligibility for university examinations without manual register alterations.

### HH. Code Constructs for Class Attendance PDF & Grade Card Generation (`core/pdf_utils.py`)
Generates standardized university monthly attendance reports and semester grade cards:
```python
def generate_monthly_attendance_pdf(student, year, month):
    """Generates official monthly attendance summary with subject-wise percentages, OD exemptions, and university header."""
    buffer = io.BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=A4, rightMargin=36, leftMargin=36, topMargin=40, bottomMargin=40)
    # Renders student metadata, subject-wise conducted/attended/leave hours, and overall attendance % badge...
    doc.build(story, canvasmaker=NumberedCanvas)
    return buffer.getvalue()

def generate_semester_grade_card_pdf(student, year_obj, semester):
    """Generates official semester grade card with course credits, internal/external marks, letter grades, SGPA, and CGPA."""
    buffer = io.BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=A4, rightMargin=36, leftMargin=36, topMargin=40, bottomMargin=40)
    # Renders university crest, student credentials, subject tabular marksheet, SGPA calculation, and Registrar signature...
    doc.build(story, canvasmaker=NumberedCanvas)
    return buffer.getvalue()
```
